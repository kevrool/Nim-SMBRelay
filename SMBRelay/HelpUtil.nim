...
proc UInt16DataLength*(start: int, payload: seq[char]): int =
    var extract: seq[string]

    if payload.len > start + 2:
        for i in payload[start..start+1]:
            extract.add(($i).toHex())
        
        result = extract.reversed().join().parseHexInt()

proc UInt32DataLength*(start: int, payload: seq[char]): int =
    var extract: seq[string]

    for i in payload[start..start+3]:
        extract.add(($i).toHex())
    
    result = extract.reversed().join().parseHexInt()

proc payloadToString*(payload: seq[char]): string =
    result = payload.join().toHex().split("00").join().parseHexStr()